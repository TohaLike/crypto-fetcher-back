...
class UserContoller {
  async registration(req, res, next) {
    try {
      ...
      res.cookie("refreshToken", userData.refreshToken, { maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true, ssecure: true, sameSite: 'lax', })
  ...
  async refresh(req, res, next) {
    try {
      ...
      res.cookie("refreshToken", userData.refreshToken, {
        maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true, secure: true, sameSite: 'lax',})
      return res.json(userData)
    } catch (e) {
      next(e)
    }
