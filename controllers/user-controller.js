...
class UserContoller {
  async registration(req, res, next) {
    try {
      ...
      res.cookie("refreshToken", userData.refreshToken, { maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true, ssecure: true, sameSite: 'none',})
  ...
  async login(req, res, next) {
    try {
      ...
      res.cookie("refreshToken", userData.refreshToken, {
        maxAge: 30 * 24 * 60 * 60 * 1000, httpOnly: true, ssecure: true, sameSite: 'none', })
      return res.json(userData)
    } catch (e) {
      next(e)
    }
